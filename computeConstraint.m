function [C2, b2, C, b, C3, b3] = computeConstraint(order, m, k_r, k_psi, t, keyframe, corridor_position, n_intermediate, corridor_width)
...
b2 = ones(2*m*(n-1)*k_r,1)*temp_eps;
constraintData;
constraintData_r;
...
for i=1:m
    for h=1:k_r
        if(i==1)
            ...
            for k=1:n-1
                if(constraintData_r(i,h,k)==temp_eps)
                    %Continuity
                    continuity(k) = true;
                end
